...
→ () 안의 숫자가 포트 번호임, 덤으로 이것도 알고 있을 것 POP(110), DNS(53)
- CSMA/CD // CSMA/CA : 충돌 탐지 // 충돌 회피
...
- IPv4와 IPv6
    ...
    8개씩 4개 블록
    유니, 멀티, 브로드 캐스트
    루프백(자기자신을 가리킴) 127.0.0.1
    255.255.255.255(마지막 주소)
    네트워크 주소 : 첫 번째 주소
...
- OLAP(On-Line Analytical Processing) 연산
 → 데이터 웨어하우스의 온라인 검색 및 분석을 지원하는 도구
    Pivoting    데이터를 분석하는 Dimension을 사용자의 요구에 따라 다양한 기준(행, 열, 페이지)으로 전환해서 볼 수 있는 기능
    Filtering   전체 데이터에서 원하는 기준만 걸러서 보여주는 기능
    Reporting   보고서에 보여지는 정보를 간단한 조작으로 원하는 형태로 보고서로 나타낼 수 있는 기능
    Slicing     다차원 데이터 항목들을 하나 혹은 그 이상의 축을 중심으로 셀들을 선택하여 조회하고 자유롭게 비교하는 기능
...
1) 제품 소프트웨어 패키징 하기
- 모듈화 
     모듈을 이용하여 소프트웨어의 성능을 향상시키거나 시스템의 디버깅, 시험, 통합 및 수정을 용이하도록하는 모듈 중심의 소프트웨어 설계 기법

- 디지털 저작권 관리(DRM Digital Right Management)
    클리어링 하우스     Clearing House 사용자에게 라이선스를 발급하고 권한을 부여하는 시스템
    DRM 구성요소        콘텐츠 제공자, 콘텐츠 분배자, 패키저, 컨트롤러, 보안 컨테이너, 클리어링하우스

 - 릴리즈 노트
    개념    소프트웨어에 대한 정보를 공유하는 문서
            내용 : 변경 사항, 개선사항 및 버그 수정 요약

    공통 작성 항목      헤더(Header) : 제품 이름, 버전번호, 날짜
                        개요(Overview) : 간략한 설명
                        문제 요약(Issue Summary) : 해결된 문제에 대한 버그 또는 개선사항 약술
                        해결(Resolution) : 개선사항, 버그를 해결하기 위한 수정사항
                        영향(Impacts) : 버전을 지원하기 위한 작업, 전제조건
    작성 순서
        모듈 식별 → 릴리즈 정보 확인 → 개요작성 → 영향도 체크 → 정식 릴리즈 노트 작성 → 추가 개선 항목 식별

2) 제품 소프트웨어 매뉴얼 작성 및 버전 등록
- 제품 소프트웨어 설치 매뉴얼 기본 작성 항목  목차 및 개요, 문서 이력 정보, 설치 메뉴얼 주석, 설치 도구의 구성
- 백업 종류 : 전체 백업 / 차등 백업 / 증분 백업